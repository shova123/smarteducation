...
$connect = mysqli_connect("localhost", "root", "", "kontext");
...
$sqlEmail.=" FROM tbl_users u";
$sqlEmail.=" INNER JOIN tbl_users_groups ug on u.user_id = ug.user_id";
$sqlEmail.=" INNER JOIN tbl_groups g on ug.group_id = g.group_id";
...
$permissionSql = mysqli_query($connect, "SELECT * FROM tbl_permissions WHERE status = 'Publish' ");
...
foreach ($uniqModuleControllers as $moduleControllersKeys => $moduleControllersValues) {
    $moduleControllerSql = mysqli_query($connect, "SELECT * FROM tbl_permissions WHERE controller LIKE '%$moduleControllersValues%' ");
