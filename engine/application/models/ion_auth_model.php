...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function end_user_hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
            	if (empty($id) || empty($password))
		{
			return FALSE;
		...
		$query = $this->db->select('password, salt')
		                  ...
		                  ->limit(1)
		                  ->order_by('end_user_id', 'desc')
		                  ->get($this->tables['end_users']);
	 ...
	 * @author Mathew
	 **/
       public function activate($user_token, $code = false) {
        $this->trigger_events('pre_activate');

        if ($code !== FALSE) {
            $userQueryID = $this->db->select('user_id')
                    ->where('activation_code', $code)
                    ->where('user_token', $user_token)
                    ->limit(1)
                    ->order_by('user_id', 'desc')
                    ->get($this->tables['users']);
            $resultID = $userQueryID->row();
            $id = $resultID->user_id;
	 ...
	 * @author Mathew
	 **/
	public function activate_end_user($user_token, $code = false)
	{
		$this->trigger_events('pre_activate');

		if ($code !== FALSE)
		{
                    $userQueryID = $this->db->select('end_user_id')
			                  ->where('activation_code', $code)
			                  ->where('user_token', $user_token)
			                  ->limit(1)
                                          ->order_by('end_user_id', 'desc')
			...
			$this->trigger_events('extra_where');
			$this->db->update($this->tables['end_users'], $data, array('end_user_id' => $id));
		}
		else
		{
			$data = array(
	 ...
	 * @author Mathew
	 **/
	public function invite_deactivate($id = NULL)
	{
		$this->trigger_events('deactivate');
	...
	public function change_password($identity, $old, $new)
	{
		...
		$old_password_matches = $this->hash_password_db($user->user_id, $old);
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		if( !isset($query->group_id) && empty($groups) )
		{
		...
		if( isset($default_group->group_id) && empty($groups) )
		{
			$groups[] = $default_group->group_id;
		}
	 ...
	 * @author Mathew
	 **/
	public function invite($email, $additional_data = array())
	{
		...
		//filter out any data passed that doesnt have a matching column in the users table
		//and merge the set user data and the additional data
		$user_data = array_merge($this->_filter_data($this->tables['end_users'], $additional_data), $data);

		$this->trigger_events('extra_set');

		$this->db->insert($this->tables['end_users'], $user_data);
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, email, user_id, password, active, last_login')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
                                  ->order_by('user_id', 'desc')
		                  ->get($this->tables['users']);
	 ...
	 * @author Mathew
	 **/
	public function end_user_login($user_identity, $password, $remember=FALSE)
	{

		$this->trigger_events('pre_login');

		if (empty($user_identity) || empty($password))
		{
			$this->set_error('login_unsuccessful');
			return FALSE;
		}
		...
		$query = $this->db->select($this->identity_column . ', username, email, end_user_id, password, active, last_login')
		                  ->where($this->identity_column, $user_identity)
		                  ->limit(1)
                                  ->order_by('end_user_id', 'desc')
		                  ->get($this->tables['end_users']);
                
		if($this->is_time_locked_out($user_identity))
		{
			//Hash something anyway, just to take up time
			$this->hash_password($password);
		...
		if ($query->num_rows() === 1)
		{
			$user = $query->row();
                        ...
                        $password = $this->end_user_hash_password_db($user->end_user_id, $password);
                        ...
                        if ($password === TRUE)
			{
				...
				$this->end_user_set_session($user);

				$this->end_user_update_last_login($user->end_user_id);

				$this->clear_login_attempts($user_identity);

				if ($remember && $this->config->item('remember_users', 'ion_auth'))
				{
					$this->remember_end_user($user->end_user_id);
				}
		...
		$this->increase_login_attempts($user_identity);
        ...
        public function linked_login($identity)
	{
...
//				}
	...
	public function clear_login_attempts($identity, $expire_period = 86400) {
		...
		return FALSE;
         ...
         //=================== login as other user start IMPORSONATE methods //Ben edmun ION IMPOESONATE LOGIN start 
        /**
	 * impersonate
	 *
	 * @return bool
	 * @author Nuri
	 **/
//       public function impersonate_real($identity,$adminLoginId=false,$login_as_code=false) {
//           //$login_as = sha1(md5(microtime()));
//        $this->load->library('ion_auth');
//        $admin = $this->ion_auth->is_admin($adminLoginId);
//        //$admin = check_admin();
//        if ($admin) {
//            $query = $this->db->select($this->identity_column . ', username, email, user_id, active, last_login')
//                    ->where($this->identity_column, $this->db->escape_str($identity))
//                    ->where('login_as_code', $login_as_code)
//                    ->limit(1)
//                    ->get($this->tables['users']);
//            if ($query->num_rows() === 1)
//            {
...
//===================================================== LOGIN AS ANOTHER USER METHODS START
    public function impersonate($newLoginEmail=null, $newLoginId=null, $adminLoginId=null,$login_as_code=null) {
        $this->load->library('ion_auth');
        
        $query = $this->db->select($this->identity_column . ', username, email, user_id, active, last_login')
                ->where($this->identity_column, $this->db->escape_str($newLoginEmail))
                ->where('login_as_code', $login_as_code)
                ->limit(1)
                ->get($this->tables['users']);
        if ($query->num_rows() === 1) {
            ...
            if ($newLoginId && is_numeric($newLoginId)) {
                if ($this->ion_auth->is_admin($adminLoginId) || $this->session->userdata('can_hijack')) {
                    if ($this->session->userdata('can_hijack') === FALSE) {
                        $this->session->set_userdata('can_hijack', $adminLoginId);
                        $this->session->set_userdata('login_as_code', $login_as_code);
                    }
                    $this->session->set_userdata('user_id', $newLoginId);
                    
                    $this->update_last_login($user->user_id);
                    $this->clear_login_attempts($newLoginEmail);
                    $this->increase_login_attempts($newLoginEmail);
                    $this->trigger_events(array('post_login', 'post_login_successful'));
                    $this->set_message('login_successful');
                    return TRUE;
                }
...
//    }
    ...
    //Ben edmun ION IMPOESONATE LOGIN end
	...
	public function num_rows()
	{
        ...
        public function users_groupType($groups = NULL,$userWhere = NULL)
	{
		...
		if (isset($groups))
		{
			...
			$or_where_in = (!empty($group_ids) && !empty($group_types)) ? 'or_where_in' : 'where_in';
			//if group name was used we do one more join with groups
			if(!empty($group_types))
			{
				$this->db->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . ' = ' . $this->tables['groups'] . '.group_id', 'inner');
				$this->db->where_in($this->tables['groups'] . '.group_type', $group_types);
			}
			if(!empty($group_ids))
			{
	...
	public function end_users()
	{
		...
		else
		{
			//default selects
			$this->db->select(array(
			    $this->tables['end_users'].'.*',
			    $this->tables['end_users'].'.end_user_id as end_user_id',
			    $this->tables['end_users'].'.end_user_id as end_user_id'
			));
		}
		...
		$this->response = $this->db->get($this->tables['end_users']);
	 ...
	 * @author Ben Edmunds
	 **/
	public function end_user($id = NULL)
	{
		$this->trigger_events('user');

		//if no id was passed use the current users id
		$id || $id = $this->session->userdata('end_user_id');

		$this->limit(1);
		$this->order_by($this->tables['end_users'].'.end_user_id', 'desc');
		$this->where($this->tables['end_users'].'.end_user_id', $id);
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as group_id, '.$this->tables['groups'].'.name,'.$this->tables['groups'].'.group_type, '.$this->tables['groups'].'.description')
	 ...
	 * @author Ben Edmunds
	 **/
	public function groups($creator_where=NULL)
	{
		...
		if (isset($this->_ion_where) && !empty($this->_ion_where))
		{
			...
			$this->_ion_where = array();
                ...
                if(!empty($creator_where)){
                    $this->db->where($creator_where);// creator condition to match groups c_user_id == creator user id
                }
	...
	public function group($id = NULL)
	{
		...
		if (isset($id))
		{
			$this->where($this->tables['groups'].'.group_id', $id);
		}

		$this->limit(1);
		$this->order_by('group_id', 'desc');

		return $this->groups();
	}

        /**
	 * groups according to creator
	 *
	 * @return object
	 * @author Ben Edmunds
	 **/
	public function creator_groups($creator_where=NULL)
	{
		$this->trigger_events('groups');
...
//		}
	 ...
	 * @author Phil Sturgeon
	 **/
	public function end_user_update($id, array $data)
	{
		$this->trigger_events('pre_update_user');

		$user = $this->end_user($id)->row();
		...
		// Filter the data passed
		$data = $this->_filter_data($this->tables['end_users'], $data);
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['end_users'], $data, array('end_user_id' => $user->end_user_id));
	...
	**/
	...
	public function update_last_login($id)
	{
		...
		$this->trigger_events('extra_where');
                $time = time();
                $date = date("Y-m-d");
		$this->db->update($this->tables['users'], array('last_login' => $date), array('user_id' => $id));
	 ...
	 * @author Ben Edmunds
	 **/
	public function end_user_update_last_login($id)
	{
		$this->trigger_events('update_last_login');
		...
		$this->db->update($this->tables['end_users'], array('last_login' => time()), array('end_user_id' => $id));
	 ...
	 * @author jrmadsen67
	 **/
	public function end_user_set_session($user)
	{

		$this->trigger_events('pre_set_session');

		$session_data = array(
		    'user_identity'             => $user->{$this->identity_column},
		    'username'             => $user->username,
		    'email'                => $user->email,
		    'end_user_id'              => $user->end_user_id, //everyone likes to overwrite id so we'll use user_id
		    'old_last_login'       => $user->last_login
		);
	...
	public function remember_user($id)
	{
        ...
        /**
	 * remember_user end user 
	 *
	 * @return bool
	 * @author Ben Edmunds
	 **/
	public function remember_end_user($id)
	{
		$this->trigger_events('pre_remember_user');
		...
		$this->db->update($this->tables['end_users'], array('remember_code' => $salt), array('end_user_id' => $id));
	...
	public function login_remembered_user()
	{
		...
		//get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', user_id, username, email, last_login')
	...
	*/
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['groups'], $additional_data), $data);
                if(is_array($additional_data)){$additional_data =$additional_data;}
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if (!empty($group_name))
		{
			...
			if(isset($existing_group->group_id) && $existing_group->group_id != $group_id)
			{
				$this->set_error('group_already_exists');
				return FALSE;
			}
        ...
        $group = $this->db->get_where($this->tables['groups'], array('group_id' => $group_id))->row();
		...
		$this->db->update($this->tables['groups'], $data, array('group_id' => $group_id));
	...
	**/
	public function delete_group($group_id = FALSE)
	{
		...
		$this->db->delete($this->tables['groups'], array('group_id' => $group_id));
        ...
        function get_users_details($user_id=null)
	{ 
		$this->db->select("*");
		...
		return $query->row(); 
         ...
         function get_end_users_details($user_id=null)
	{ 
		$this->db->select("*");
		$this->db->where("end_user_id", $user_id); 
	...
	{		
        ...
        function insert_profile( $table, $array)
	{ 	
		$this->db->set( $array );
