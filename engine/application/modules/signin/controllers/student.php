...
//            $where="where s.board_id=".$board_id." AND s.level_id=".$level_id." AND s.stream_id=".$stream_id." AND s.course_id=".$course_id ." AND s.department_id=".$department_id." AND s.year=".$year." AND s.semester=".$semester ." AND q.appeared_year < ".$date ;
             $where="where s.board_id=".$board_id." AND s.level_id=".$level_id."   AND q.appeared_year < ".$date ;
             if(!empty($stream_id) && $stream_id >0){
                      $where .=" AND s.stream_id=".$stream_id." ";
                  }    
...
//                 $where1 .="subject_id='$subject' AND ";
           ...
           if(isset($_POST['subject'])){
            ...
            $data['questions']=$this->db->query("select * from hya_data_question where $where1  subject_id=$subjectId AND status=1 order by created_date DESC")->result_array();
...
//                 $where1 .="appeared_year='$date' AND ";
            ...
            $data['chapters']=$this->db->query("select * from hya_course_chapter". $where)->result_array();
            $data['level']=$this->db->query("select * from hya_course_level where  level_id=$level_id")->row();
            $data['subjects']=$this->db->query("select * from hya_course_subject where board_id=$board_id AND level_id=$level_id AND stream_id=$stream_id AND course_id=$course_id AND department_id=$department_id AND year=$year AND semester=$semester AND status=1")->result();
           
//          

            $data['questions']=$questions=$this->db->query("select * from hya_data_question where chapter_id=$chapterId AND subject_id=$subjectId AND $where1 status=1 order by created_date DESC")->result_array();
            $questions=$this->db->query("select * from hya_data_question where status=1")->result_array();
            $data['marks'] = array_unique(array_column($questions, 'mark'));
