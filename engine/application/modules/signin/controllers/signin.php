  ...
  */
  function get_fb_contents($url) {
    ...
    if (isset($_GET['code']) AND !empty($_GET['code'])) {
     ...
     $access_token= $this->get_fb_contents("https://graph.facebook.com/oauth/access_token?client_id=816999125058531&redirect_uri=" . urlencode(base_url('signin/facebooklogin')) ."&client_secret=57dc22e2b5ed8cdabab555cf4bf630e1&code=" . urlencode($code));
     //echo $access_token;die;
     redirect('signin/facebooklogin?'.$access_token);
    }
...
//                echo "<pre>";
//                print_r($fbuser_info);die;
            $groupData = @$group_id;
            $user_token = md5(uniqid(mt_rand(), True));
            $linkedinID = $fbuser_info['id'];
            $first_name = $fbuser_info['first_name'];
            $second_name = $fbuser_info['last_name'];
            $username= strtolower($first_name)."_".strtolower($second_name);
            $password='';
            $email = $fbuser_info['email'];
//            $image_url = $fbuser_info['public_profile'];
...
//                file_put_contents($image_path, $get_image);
            ...
            if($email_exist == true){
                ...
                $groupTypeDashboardName = $user_group_details->group_type;
                    if($exist_linkedinID == $linkedinID){// already logged /register as candidate (users) through linkedin
                        ...
                        $this->session->set_flashdata('error_message', "Account already member in Student.Please try with different  Account");
                    }else{
                        $this->session->set_flashdata('error_message', "Account already member in Student Through system.Please try with different  Account");//You are trying to log in as a $tryTo. Please switch to $var Login Tab.
    ...
    if ($this->form_validation->run() == FALSE) {
       $this->template->load('template_front', 'login'); // loading default view.
    }
    ...
    function user_dashboard() {
        ...
        if (!$this->ion_auth->logged_in()) {
            redirect('signin/login', 'refresh');
        } else if ($this->ion_auth->has_permission("signin/signin", 'user_dashboard', $this->ion_auth->get_user_id())) {
            $data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
            if ($this->session->userdata('user_id') != "") {
                $userID = $this->session->userdata('user_id');
            ...
            $this->_render_template_front_dashboard("user_dashboard", $data);
        } else {
            $this->session->set_flashdata('error_message', 'You Dont have permission to view this page.');
            redirect("signin/login", 'refresh');
        }
...
//        }
    ...
    function login() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $this->_render_template_front('signin/login', $data);
        }
//        } else { 
//            
//            return show_error('You Dont have permission to view this page.');
...
//       
        ...
        $users_details = $this->users_model->get_byId('hya_users', 'user_token', $user_token);
        $id = $users_details->user_id;
        $category_id = $users_details->category_id;
        if($this->input->post('profileEditSave')){
//             print_r($_POST);die;
            $update=array(
                ...
                'home_image'=>$_POST['fileList']
            );
            $this->general_db_model->update('users',$update,'user_token ="'.$user_token.'" ');
            $update2=array(
             ...
             $this->general_db_model->update('users_eduinfo',$update2,'user_id = '. $id);
//             $this->general_db_model->update('users', $post, 'user_id = ' . $id);
        }
        if (!$this->ion_auth->logged_in()) {
...
//            print_r($data['details']);die;
           ...
           $studentDetail=$this->general_db_model->getById('users_eduinfo','user_id',$loginUserID);
            ...
            $boardId=$studentDetail->board_id;
            $levelId=$studentDetail->level_id;
            $streamId=$studentDetail->stream_id;
            $courseId=$studentDetail->course_id;
...
//           print_r($studentDetail);die;
            ...
            $data['s_board']=$boardId;//$this->general_db_model->getById('hya_course_board','board_id',$boardId);
            $data['s_level']=$levelId;//$this->general_db_model->getById('hya_course_level','level_id',$levelId);
            $data['s_stream']=$streamId;//$this->general_db_model->getById('hya_course_stream','stream_id',$streamId);
            $data['s_course']=$courseId;//$this->general_db_model->getById('hya_course_course','course_id',$courseId);
            $data['s_department']=$departmentId;//$this->general_db_model->getById('hya_course_department','department_id',$departmentId);
            $data['s_year']=$year;
            $data['s_semester']=$semester;
            ...
            $this->_render_template_front_dashboard("student/profile", $data);
        } 
...
//===================================================== LOGIN AS ANOTHER USER METHODS END
    ...
    function forgot_password() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
            $this->_render_template_front('signin/forgot_password', $data);
        } else {
                        ...
                        if ($this->form_validation->run() === TRUE) {
                            ...
                            if ($this->ion_auth->update($user->user_id, $post)) {
                                ...
                                if ($this->input->post('board_id')) {
                                    $value['user_id'] = $id;
                                    $value['board_id'] = $this->input->post('board_id');
                                    $value['level_id'] = $this->input->post('level_id');
                                    $value['stream_id'] = $this->input->post('stream_id');
                                    $value['course_id'] = $this->input->post('course_id');
                                    $value['department_id'] = $this->input->post('department_id');

                                    $valueExist = $this->general_db_model->value_exist('users_eduinfo', 'user_id', $id);

                                    if ($valueExist == TRUE) {
                                        $this->general_db_model->update("users_eduinfo", $value, "user_id ='$id'");
                                    } else {
                                        $this->general_db_model->insert("users_eduinfo", $value);
                                    }
                                }
...
//                            }
        ...
        $data['boards'] = $this->signin_model->get_results("course_board", 'status', '1', 'board_name');
        $data['levels'] = $this->signin_model->get_results("course_level", 'status', '1', 'level_name');
        $data['streams'] = $this->signin_model->get_results("course_stream", 'status', '1', 'stream_name');
        $data['courses'] = $this->signin_model->get_results("course_course", 'status', '1', 'course_name');
        $data['departments'] = $this->signin_model->get_results("course_department", 'status', '1', 'department_name');
//        $data['subjects'] = $this->signin_model->get_results("course_subject", 'status', '1', 'subject_name');
//        $data['chapters'] = $this->signin_model->get_results("course_chapter", 'status', '1', 'chapter_name');
//        $data['units'] = $this->signin_model->get_results("course_unit", 'status', '1', 'unit_name');
//        $data['subunits'] = $this->signin_model->get_results("course_subunit", 'status', '1', 'subunit_name');
