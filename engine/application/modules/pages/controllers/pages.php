...
class Pages extends CI_Controller {
    ...
    function page_list() {
        ...
        } else if ($this->ion_auth->has_permission("pages/pages", 'page_list', $this->ion_auth->get_user_id())) {
    ...
    function contents($link = null) {
        ...
        if (!$this->ion_auth->logged_in()) {
            redirect('signin/login', 'refresh');
        } else if ($this->ion_auth->has_permission("pages/pages", 'contents', $this->ion_auth->get_user_id())) {
    ...
    function add_page($link = null) {
        if (!$this->ion_auth->logged_in()) {
            redirect('signin/login', 'refresh');
        } else if ($this->ion_auth->has_permission("pages/pages", 'add_page', $this->ion_auth->get_user_id())) {
    ...
    function edit_page($id, $link = null) {
        if (!$this->ion_auth->logged_in()) {
            redirect('signin/login', 'refresh');
        } else if ($this->ion_auth->has_permission("pages/pages", 'edit_page', $this->ion_auth->get_user_id())) {
            if ($this->input->post('submitDetail')) {
                $this->add_edit($id);
                die();
            }
    ...
    function delete_page($id) {
        if (!$this->ion_auth->logged_in()) {
            redirect('signin/login', 'refresh');
        } else if ($this->ion_auth->has_permission("pages/pages", 'delete_page', $this->ion_auth->get_user_id())) {
...
//                        $this->general_db_model->delete('package', 'category_id = '.$id);
    ...
    function news_list(){
        ...
        $data['news'] = $this->general_db_model->get_results("news");
    ...
    function news_edit($id) {
        if ($this->input->post('submitDetail')) {
            $this->add_edit_news($id);
            die();
        }
        $data['details'] = $this->general_db_model->getById('news', 'id', $id);
    ...
    function add_edit_news($id = NULL) {
//			$not = array('submitDetail','fileList');
//			$post = $this->mylibrary->get_post_array($not);
        ...
        $post['date'] =date('Y-m-d H:i:s',strtotime($this->input->post('date')));
//        $post['order'] = $this->input->post('order');
        $post['status'] = $this->input->post('status');
        $post['home_image'] = $this->input->post('home_image');
//echo $post['date'];die;
        ...
        //dumparray($post);
        if ($id) {
            $this->general_db_model->update('news', $post, 'id = ' . $id);
            $this->session->set_flashdata('display_message', 'News successfully updated.');
        } else {
            $this->general_db_model->insert('news', $post);
            $this->session->set_flashdata('display_message', 'News successfully added.');
        }
        redirect("pages/news_list");
        // $this->template->load('template','about_form', @$data);
    }

    function news_delete($id) {
        $this->general_db_model->delete('news', 'id = ' . $id);
//                        $this->general_db_model->delete('region', 'category_id = '.$id);
//                        $this->general_db_model->delete('package', 'category_id = '.$id);
        $this->session->set_flashdata('display_message', 'Category successfully deleted.');
        redirect("pages/news_list");
    ...
    function delete_news_image() {
        $img = $this->input->post('imgName');
        $imgpath = $this->config->item('uploads') . 'news/' . $img; //echo $imgpath; exit;
        if (@file_exists($imgpath)) {
            unlink($imgpath);
            echo 'Image successfully deleted!';
        } else {
     ...
     function news_update_status() {
        ...
        if ($status == 'Publish')
            ...
            $data['status'] = 'Publish';
        $this->general_db_model->update('news', $data, 'id = ' . $id);
    ...
    function videos(){
        ...
        $data['videos'] = $this->general_db_model->get_results("video_link");
        $data['current'] = 8;
        $data['dropsub'] = 2;
        $this->template->load('template', 'video', $data);
    }
    ...
    function add_video() {
        ...
        $this->template->load('template', 'video_form', @$data);
    }

    function edit_video($id) {
        if ($this->input->post('submitDetail')) {
            $this->add_edit_video($id);
            die();
        }
        $data['details'] = $this->general_db_model->getById('video_link', 'id', $id);
     ...
     function add_edit_video($id = NULL) {

        $post['video_title'] = $this->input->post('video_title');
        $post['video_slug'] = $this->input->post('video_slug');
...
//        $post['order'] = $this->input->post('order');
        ...
        //dumparray($post);
        if ($id) {
            $this->general_db_model->update('video_link', $post, 'id = ' . $id);
            $this->session->set_flashdata('display_message', 'Video successfully updated.');
        } else {
            $this->general_db_model->insert('video_link', $post);
            $this->session->set_flashdata('display_message', 'Video successfully added.');
        }
        redirect("pages/videos");
    }
     function video_delete($id) {
        $this->general_db_model->delete('video_link', 'id = ' . $id);
        $this->session->set_flashdata('display_message', 'Video successfully deleted.');
        redirect("pages/videos");
    }
