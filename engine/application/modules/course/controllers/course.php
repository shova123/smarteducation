...
//                if(count($exist)<= 0){
                for ($i = 0; $i < $subject; $i++) {
                    $insert['token']=md5(uniqid(mt_rand(), True));
                    $insert['subject_name'] = $subject_name = $_POST['subject_name'][$i];
                    $insert['subject_slug'] =$this->slugify($subject_name, "-");
...
//                );
                ...
                for ($i = 0; $i < $subject; $i++) {
                     $update['token']=md5(uniqid(mt_rand(), True));
                    $update['subject_name'] = $subject_name = $_POST['subject_name'][$i];
                    $update['subject_slug'] =$this->slugify($subject_name, "-");
...
//                    redirect('course/list_chapter');
                    ...
                    $insert['chapter_slug'] =$this->slugify($chapter_name, "-");
                    $insert['status'] = $status = $this->input->post('status');
                    $insert['user_id'] = $this->user_id;
                    $insert['created_at'] = date("Y-m-d H:i:s");
                    //print_r($insert);die;
...
//                $insert['chapter_slug'] =$this->slugify($chapter_name, "-");
                $insert = array(
                    ...
                    'chapter_slug' => $this->slugify($_POST['chapter_name'][0], "-"),
                    'board_id' => $_POST['board_id'],
                    'level_id' => $_POST['level_id'],
                    'stream_id' => $_POST['stream_id'],
                    'course_id' => $_POST['course_id'],
