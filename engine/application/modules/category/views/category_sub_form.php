...
<script>
    $(document).ready(function () {
        $("#subcategory_title").stringToSlug();
    });
</script>
    ...
    $(function () {
        $('#home_image').uploadifive({
            ...
            'formData': {
                ...
                'targetFolder': '/uploads/sub_category/',
            ...
            'onUploadComplete': function (file, data, response) {
                ...
                $('.imagethumbs-form').prepend('<div class="imagethumb-form additional-file-input" id="add-image1" > <a class="close-msg" title="Delete" id="deleteImg">Delete</a> <a href="#" title="'+data+'" class="img-wrap"><img src="'+"<?php echo base_url();?>"+'gears/admin/createThumb/create_thumb.php?src='+imagePath+'uploads/sub_category/'+data+'&w=150&h=150" alt="'+data+'" /></a></div>');
...
//THIS FUNCTION IS TRIGGERED WHILE UPLOADED IMAGE, IS REQUIRED TO DELETE:
    $(document).on('click', 'a#deleteImg', function () {
        ...
        delete_image(_img);
        $.post("<?php echo admin_url("category/delete_category_sub_image");?>", {imgName: _img},
        function (data) {
            ...
            <div class="box box-color box-bordered">
                ...
                <div class="box-content nopadding">
                    <form action="" method="POST" class="form-horizontal form-bordered form-validate" enctype='multipart/form-data' >
                                ...
                                <select name="category_id" id="category_id" class='chosen-select form-control'>
                                    ...
                                    if(!empty($category_list)){
                                        foreach($category_list as $catList){
                                            $categoryId= $catList->category_id;
                                            $categoryTitle = $catList->category_title;
                                            $categorySlug = $catList->category_slug;
                                    ?>
                                    <option value="<?php echo $categoryId;?>" <?php if (($isEdit) && ($details->category_id == $categoryId)) echo "selected"; ?>> <?php echo $categoryTitle;?></option>
                                    <?php }}?>
                                </select>
                        ...
                        <div class="form-group">
                                <label for="textfield" class="control-label col-sm-2">Sub Category Title</label>
                                <div class="col-sm-10">
                                        <input type="text" name="subcategory_title" id="subcategory_title" value="<?php if (@$isEdit) echo $details->subcategory_title; ?>" class="form-control">
                                </div>
                        </div>
                        <div class="form-group">
                                <label for="textfield" class="control-label col-sm-2">Sub Category Slug</label>
                                <div class="col-sm-10">
                                    <input type="text" name="subcategory_slug" id="permalink" value="<?php if (@$isEdit) echo $details->subcategory_slug; ?>" class="form-control slug" readonly>
...
<!--                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                <select name="type" id="type" class='chosen-select form-control'>
                                    <option value="" <?php if (($isEdit) && ($details->category_id == '')) echo "selected"; ?>> Category Type</option>
                                    <option value="popular_certifications" <?php if (($isEdit) && ($details->type == 'popular_certifications')){echo "selected";}?>> Popular Certifications </option>
                                    <option value="updated_exams" <?php if (($isEdit) && ($details->type == 'updated_exams')){echo "selected";}?>> Updated Exams </option>
                                </select>
                                <span class="help-block">Choose option to publish or unpublish the page.</span>
                            </div>
                        </div>-->
...
<!--                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <?php  if(!empty($isEdit)){$checkboxd = explode("||",$details->display_category_id);}?>
                                <label class="checkbox-inline">
                                    <input type="checkbox" value="default" id="inlinecheckbox1" class='icheck-me' data-skin="square" data-color="red" name="display_category_id[]" <?php if(@$isEdit){ foreach($checkboxd as $chkd){ if($chkd =='default'){?> checked<?php }}}else{echo "checked";}?>>
                                    <span class="custom-checkbox"></span> Default 
                                </label>
                                <label class="checkbox-inline">
                                    <input type="checkbox" value="popular" id="inlinecheckbox1" class='icheck-me' data-skin="square" data-color="red" name="display_category_id[]" <?php if(@$isEdit){ foreach($checkboxd as $chkd1){ if($chkd1 =='popular'){?> checked<?php }}}?>>
...
<!--                        <div class="form-group">
                        ...
                        <div class="form-group">
                                <label for="textfield" class="control-label col-sm-2">Sub Category Description</label>
                                <div class="col-sm-10">
                                        <textarea name="subcategory_description" id="subcategory_description" class="form-control ckeditor"><?php if (@$isEdit) echo $details->subcategory_description; ?></textarea>
                                        <span class="help-block">This is page slug recognization</span>
                                </div>
                        </div>
                        <div class="form-group">
                                            ...
                                            foreach ($img as $i) {
                                                ...
                                                <div class="imagethumb-form additional-file-input" id="add-image1">
                                                    ...
                                                    <a href="#" title="<?php echo $i;?>" class="img-wrap">
                                                        <img src="<?php echo base_url();?>gears/admin/createThumb/create_thumb.php?src=<?php echo ROOT;?>uploads/sub_category/<?php echo $i;?>&w=150&h=150" />
                                                    </a>  
                                                </div>
                                                <?php
                                            }
