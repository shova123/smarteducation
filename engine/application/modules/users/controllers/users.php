...
//            if (isset($response['success']) && $response['success'] === true) {
                if ($this->form_validation->run() == true) {
                    ...
                    $countFullname = count($explode_fullname);
                    $lastName = '';
                    $lastUserName = '';
                    for ($i = 0; $i < $countFullname; $i++) {
                        $indexing = $i + 1;
                    $first_name = @$explode_fullname[0];
                    $last_name = @$explode_fullname[$indexing];
                        $lastName .= "$last_name ";
                        $lastUserName .= "$last_name" . "_";
                    }

                    $last_name = trim($lastName, ' ');$trimLastUserName = trim($lastUserName, '_');
                    $username = strtolower($first_name) . "_" . strtolower($trimLastUserName);
...
//                        'status' => $this->input->post('status'),
                ...
                if ($this->form_validation->run() == true && $this->ion_auth->register($username, $password, $email, $additional_data, $groupData)) {    
                    ...
                    $data['full_name'] = array('name' => 'full_name','id' => 'full_name','type' => 'text','value' => $this->form_validation->set_value('full_name'));
                    $data['email'] = array('name' => 'email','id' => 'email','type' => 'text','value' => $this->form_validation->set_value('email'));
                    $this->session->set_flashdata('error_message', (validation_errors() ? validation_errors() : ($this->ion_auth->errors() ? $this->ion_auth->errors() : $this->session->flashdata('message'))));
                }
//            }
