...
//                $email_activation_messages = sprintf(lang('email_activate_subheading'), anchor('signin/activate/'. $user_token .'/'. $activation, lang('email_activate_link')));
                //$message = $this->load->view($this->config->item('email_templates', 'ion_auth') . $this->config->item('email_activate', 'ion_auth'), $data, true);
                $email_activation_heading = sprintf(lang('email_activate_heading'), $identity);
                $email_activation_messages = sprintf(lang('email_activate_subheading'), anchor("signin/signup_complete/$user_token/$activation", lang('email_signup_activate_link')));
                //$message = "<html><body><h1>$email_activation_heading</h1><p>$email_activation_messages</p></body></html>";
                $message = "<!DOCTYPE html>
                            <html lang='en'>
                                <head>
                                    <title>Kontext Email</title>
                                </head>
                                                    ...
                                                    <table bgcolor='#1f1f1f'>
                                                        <tr>
                                                            <td style='width:100%; margin:0 auto; padding:10px;'>
                                                                <img src='http://kontext.com.au/gears/front/images/kontext2_white.png' width='150' style='margin:0 auto' />
                                                            </td>
                                                        </tr>
                                                    </table>
                                                </div>
...
//            return FALSE;
	...
	public function has_permission($controller, $required_permissions_actionName, $user_id = NULL)
	{
		...
		//$role_id= $this->_CI->session->userdata('role_id');
                //$user_roles=$this->_CI->db->query("select * from hya_groups where group_id=$role_id")->row_array();
