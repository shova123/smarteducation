...
class Barcodes {
    ...
    public function getBarcodePNG($code, $type, $w=3, $h=3, $color=array(0, 0, 0)) {
        ...
        /**
     * Return a base64 data image representation of barcode (requires GD ).
     * Created & Modified By Yacine REZGUI (yacine.rezgui@gmail.com).
     * <li>$arrcode['code'] code to be printed on text label</li>
     * <li>$arrcode['num_rows'] required number of rows</li>
     * <li>$arrcode['num_cols'] required number of columns</li>
     * <li>$arrcode['bcode'][$r][$c] value of the cell is $r row and $c column (0 = transparent, 1 = black)</li></ul>
     ...
     * @param $h (int) Height of a single rectangle element in pixels.
     * @param $color (array) RGB (0-255) foreground color for bar elements (background is transparent).
     * @return base64 data or false in case of error.
     * @public
     */
    
        public function getBarcodeB64($code, $type, $w = 3, $h = 3, $color = array(0, 0, 0)) {
        ...
        $height = ($this->barcode_array['num_rows'] * $h);
        if (function_exists('imagecreate')) {
            $b64 = imagecreate($width, $height);
            //$png = imagecreate(3, 3);
            $bgcol = imagecolorallocate($b64, 255, 255, 255);
            imagecolortransparent($b64, $bgcol);
            $fgcol = imagecolorallocate($b64, $color[0], $color[1], $color[2]);
        } else {
            return false;
        }
        // print barcode elements
        ...
        for ($r = 0; $r < $this->barcode_array['num_rows']; ++$r) {
            ...
            for ($c = 0; $c < $this->barcode_array['num_cols']; ++$c) {
                if ($this->barcode_array['bcode'][$r][$c] == 1) {
                    // draw a single barcode cell
                        imagefilledrectangle($b64, $x, $y, ($x + $w), ($y + $h), $fgcol);
                }
                $x += $w;
            }
            $y += $h;
    ...
    /**
     * $imgName for storing the image name in the folder that is supplied
     ...
     * @public
     */
    public function getBarcodePNGPath($imgName, $code, $type, $w=3, $h=3, $color=array(0, 0, 0)) {
        ...
        if (function_exists('imagecreate')) {
            ...
            $bgcol = imagecolorallocate($png, 255, 255, 255);
//            echo "<pre>";
//            echo $code."<br/>".$type."<br/>".$w."<br/>".$h."<br/>".$color."<br/>".$bgcol;die;
            imagecolortransparent($png, $bgcol);
            $fgcol = imagecolorallocate($png, $color[0], $color[1], $color[2]);
        } elseif (extension_loaded('imagick')) {
            $imagick = true;
        ...
        $save_file = $this->checkfile($this->save_path.$imgName . ".png");
