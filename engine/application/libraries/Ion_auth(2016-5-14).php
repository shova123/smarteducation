        ...
        if ($this->ion_auth_model->forgotten_password($identity)) {   //changed
            ...
            if ($user) {
                ...
                if (!$this->config->item('use_ci_email', 'ion_auth')) {
                    ...
                    $email_forget_password_heading = sprintf(lang('email_forgot_password_heading'), $identity);
                    $email_forget_password_messages = sprintf(lang('email_forgot_password_subheading'), anchor("signin/reset_password/$forgotten_password_code", lang('email_forgot_password_link')));
                    
                    $emailForgetPasswordHeader = $this->config->item('email_forget_password_header', 'ion_auth');
                    $emailForgetPasswordFooter = $this->config->item('email_forget_password_footer', 'ion_auth');
...
//                    }
                    ...
                    $message = "<!DOCTYPE html>
                            <html lang='en'>
                                <body bgcolor='#FFFFFF' style='max-width:600px; margin:0 auto; font-family:Tahoma;'>
                                    <table bgcolor='#1f1f1f' style='width:100%'>
                                        <tr>
                                            <td></td>
                                            <td>
                                                <div class='content'>
                                                    <table bgcolor='#1f1f1f'>
                                                        <tr>
                                                            <td style='width:100%; margin:0 auto; padding:10px;'>
                                                                $emailForgetPasswordHeader
                                                            </td>
                                                        </tr>
                                                    </table>
                                                </div>
                                            ...
                                            <td class='container' bgcolor='#FFFFFF'>
                                                <div class='content'>
                                                    <table>
                                                        <tr>
                                                            <td>
                                                                <h2>$email_forget_password_heading</h2>

                                                                <p>$email_forget_password_messages</p>
                                                            </td>
                                                        </tr>
                                                    </table>
                                                </div>					
                ...
                $this->email->from($this->config->item('admin_email', 'ion_auth'), $this->config->item('email_title', 'ion_auth'));
                $this->email->to($user->email);
                $this->email->subject($this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_forgotten_password_subject'));
                $this->email->message($message);
    ...
    public function forgotten_password_complete($code) {
        ...
        if ($new_password) {
            ...
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
                ...
                $email_new_password_heading = sprintf(lang('email_new_password_heading'), $identity);
                $email_new_password_messages = sprintf(lang('email_new_password_subheading'), $new_password);
                    
                    $emailNewPasswordHeader = $this->config->item('email_new_password_header', 'ion_auth');
                    $emailNewPasswordFooter = $this->config->item('email_new_password_footer', 'ion_auth');
                                            ...
                                            <td class='container' bgcolor='#FFFFFF'>
                                                <div class='content'>
                                                    <table>
                                                        <tr>
                                                            <td>
                                                                <h2>$email_new_password_heading</h2>

                                                                <p>$email_new_password_messages</p>
                                                            </td>
                                                        </tr>
                                                    </table>
                                                </div>					
...
//                $email_activation_messages = sprintf(lang('email_activate_subheading'), anchor('signin/activate/'. $user_token .'/'. $activation, lang('email_activate_link')));
                ...
                $emailActivationHeader = $this->config->item('email_activate_header', 'ion_auth');
                $emailActivationFooter = $this->config->item('email_activate_footer', 'ion_auth');
                $message = "<!DOCTYPE html>
                            <html lang='en'>
                                <body style='max-width:600px; margin:0 auto; font-family:Tahoma;'>
                                            ...
                                            <td class='container' bgcolor='#FFFFFF'>
                                                <div class='content'>
                                                    <table>
                                                        <tr>
                                                            <td>
                                                                <h2>$email_activation_heading</h2>

                                                                <p>$email_activation_messages</p>
                                                            </td>
                                                        </tr>
                                                    </table>
                                                </div>					
                ...
                $config['mailpath'] = "/usr/bin/sendmail"; // or "/usr/sbin/sendmail"
                $config['protocol'] = "smtp";
                $config['smtp_host'] = "mail.smartshikshya.com";
                ...
                $this->email->clear();
                $this->email->initialize($config);
                $this->email->from($this->config->item('admin_email', 'ion_auth'), $this->config->item('email_activation_title', 'ion_auth'));
                $this->email->to($email);
                $this->email->subject($this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_activation_subject'));
                $this->email->message($message);
...
//            return FALSE;
    ...
    /*
     * Functoin to register new linkedin user only one time
     * 
     */
    public function linkedin_register($username, $password, $email, $additional_data = array(), $group_ids = array()) { //need to test email activation
...
//                @$this->load->helper(array('email'));
//                @$this->load->library(array('email'),$configr);

                //$this->email->initialize($configr);
//                $this->email->from($this->config->item('admin_email', 'ion_auth'), $this->config->item('site_title', 'ion_auth'));
//                $this->email->to($email);
//                $this->email->subject($this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_activation_subject'));
//                $this->email->message($message);
...
//            return FALSE;
     ...
     * @author jrmadsen67
     * */
    public function get_end_user_id() {
     ...
     * @author jrmadsen67
     * */
    public function login_end_user_details($loginEndUserId=null) {
        $user_id = $this->session->userdata('end_user_id');
        if (!empty($user_id)) {
            $user_details = $this->ion_auth_model->get_end_users_details($user_id);
            return $user_details;
        }elseif (!empty($loginEndUserId)) {
            $user_details = $this->ion_auth_model->get_end_users_details($loginEndUserId);
            return $user_details;
        }
        return null;
    }
