...
$(function(){
	$('#home_image').uploadify({
		formData     	: {
					...
					'targetFolder' : '/uploads/footer_images/',
	...
	/*	//Default as provided by site: ITS ALWAYS SAME, THE ORDER NEEDS TO REMAIN SAME, NO MATTER THE NAME OF VARIABLE!
		onUploadSuccess : function(file, data, response) {
  		...
  		onUploadSuccess : function(file, data, response) {	//alert(file.name);
					...
					$('.imagethumbs-form').prepend('<div class="imagethumb-form additional-file-input" id="add-image1" > <a class="close-msg" title="Delete" id="deleteImg">Delete</a> <a href="#" title="'+data+'" class="img-wrap"><img src="'+"<?php echo base_url();?>"+'assets/createThumb/create_thumb.php?src='+imagePath+'uploads/footer_images/'+data+'&w=150&h=150" alt="'+data+'" /></a></div>');
					//alert($('#fileList').val());					
...
$(function(){
	$('a#deleteImg').live('click',function(){		
		...
		delete_image(_img);
		$.post("<?php echo admin_url("links/image_delete_image");?>",{imgName:_img},
		function(data){
                ...
                <div class="porlets-content">
                    <div class="form-horizontal row-border" ><!--form start-->
                            ...
                            <div class="form-group">
                                <label class="col-sm-2 control-label"><strong>Image Title</strong></label>
                                <div class="col-sm-4">
                                <input type="text" class="form-control" name="image_title" id="page_title" value="<?php if(@$isEdit) echo $details->image_title;?>"/>
                                </div>
                            </div><!--/form-group-->
                            <div class="form-group">
                                <label class="col-sm-2 control-label"><strong>Image Link</strong></label>
                                <div class="col-sm-4">
                                <input type="text" class="form-control" name="image_link" id="page_title" value="<?php if(@$isEdit) echo $details->image_link;?>"/>
                                </div>
                            </div><!--/form-group-->
                            ...
                            ?>
                                <div class="imagethumb-form additional-file-input" id="add-image1">
                                        ...
                                        <a href="#" title="<?php echo $i;?>" class="img-wrap">
                                            <img src="<?php echo base_url();?>assets/createThumb/create_thumb.php?src=<?php echo ROOT;?>uploads/footer_images/<?php echo $i;?>&w=150&h=150" />
                                        </a>  
                                </div>
                            <?php endforeach;} }?>
                            </div>
